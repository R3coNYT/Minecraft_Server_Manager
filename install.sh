...
if ! [[ "$EXISTING_USERS" =~ ^[0-9]+$ ]]; then
  ...
  warn "No account will be created; add one if needed: sudo msm createadmin NAME"
  EXISTING_USERS=1
fi
...
if [[ "$SKIP_ADMIN" -eq 1 ]]; then
  ...
  if run_as_msm "$VENV/bin/python" -m msm.cli createadmin "$ADMIN_NAME"; then
    ...
    warn "Try again once the installation is over: sudo msm createadmin NAME"
  fi
fi
...
# The `msm` command: MSM's command line, run exactly like the service — as its
# user, with its configuration. `sudo msm createadmin NAME` creates an account.
cat > /usr/local/bin/msm <<WRAPPER
#!/usr/bin/env bash
# Generated by install.sh: MSM's command line, run as the service runs it.
set -euo pipefail
if [[ \$EUID -ne 0 ]]; then
  exec sudo "\$0" "\$@"
fi
exec runuser -u ${MSM_USER} -- bash -c 'set -a; . "\$1"; set +a; cd "\$2"; shift 2; exec "\$@"' \\
  _ "${ENV_FILE}" "${INSTALL_DIR}/backend" "${VENV}/bin/python" -m msm.cli "\$@"
WRAPPER
chmod 755 /usr/local/bin/msm
ok "Command installed: sudo msm createadmin NAME"
...
cat <<EOF
  ...
  Create an account  sudo msm createadmin NAME
